...
const TABS = [
  ...
  { id: 'zamanlanmis', label: 'Zamanlanmış' },
  { id: 'webhook', label: 'Webhook' },
  { id: 'hatali', label: 'Hatalı' },
  { id: 'run_gecmisi', label: 'Run Geçmişi' },
  { id: 'maliyet', label: 'Maliyet' },
];

export default function OtomasyonlarModule() {
  ...
  return (
    <div className="module-transition space-y-5">
      <div>
        ...
        <h1 className="mihenk-module-title flex items-center gap-2"><Cog className="w-6 h-6 text-[hsl(var(--mihenk-red))]" /> Otomasyonlar</h1>
        <p className="text-sm text-muted-foreground mt-1">Cron, tetikleyici ve tekrarlı ajan işleri</p>
      </div>
      ...
      <div className="mihenk-card p-6 text-center py-12 text-muted-foreground">
        <Cog className="w-8 h-8 mx-auto mb-2 opacity-40" />
        <p className="text-sm">Otomasyonlar modülü hazırlanıyor</p>
      </div>
    </div>
  );
}
